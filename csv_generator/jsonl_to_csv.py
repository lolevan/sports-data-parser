...
def sanitize_column_name(type_name, line):
    ...
    """
    type_clean = type_name.replace(" ", "_").replace("/", "_").replace("-", "_")
    line_clean = str(line).replace(".", "_").replace("-", "m")
...
def save_to_csv(events, output_path):
    ...
    basic_columns = ['event_id', 'match_name', 'start_time', 'home_team', 'away_team', 'league', 'country', 'sport', 'bookmaker']
